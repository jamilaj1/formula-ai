...
def render_auth():
    ...
    with tab1:
        ...
        e_log = st.text_input("Professional Email", key="unique_login_email_final")
        p_log = st.text_input("Password", type="password", key="unique_login_pass_final")
        if st.button("Authenticate & Enter", key="unique_login_btn_final"):
            try:
    ...
    with tab2:
        n_reg = st.text_input("Full Name", placeholder="e.g., Jamil Abduljalil", key="unique_reg_name_final")
        e_reg = st.text_input("Email", key="unique_reg_email_final")
        p_reg = st.text_input("Security Password", type="password", key="unique_reg_pass_final")
        if st.button("Initialize Pro Account", key="unique_reg_btn_final"):
            try:
                # التسجيل وإضافة السجل لحل خطأ PGRST205
                auth_res = supabase.auth.sign_up({"email": e_reg, "password": p_reg, "options": {"data": {"full_name": n_reg}}})
                # محاولة إدخال السجل في الجدول الذي أنشأته بنجاح
...
def render_main():
    with st.sidebar:
        ...
        if st.button("Logout 🚪", key="global_logout_final"):
    ...
    if prompt := st.chat_input("Enter your manufacturing query...", key="global_chat_input_final"):
